...
class FluoTrackApp:
    ...
    def _setup_gui(self):
        ...
        tk.Entry(output_frame, textvariable=self.output_dir, width=20).pack(side='left', padx=5)
        tk.Button(output_frame, text="Browse", command=self._browse_output).pack(side='left')
    ...
    def _track_brightest_point(self):
        ...
        try:
            while True:
                ...
                cv2.putText(
                    frame, f"FPS: {self.tracker.fps:.1f}",
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
                    1, (0, 255, 0), 2
                )
                cv2.putText(
                    frame, f"Brightness: {result['intensity']:.0f}",
        ...
        try:
            while True:
                ...
                for i, region in enumerate(regions[:5]):  # Show top 5
                    ...
                    # Label
                    cv2.putText(
                        frame, f"#{i+1}: {region['intensity']:.0f}",
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('fluotrack.log'),
