...
class BrightnessAnalyzer:
    ...
    def plot_brightness_trend(self, output_file: Optional[str] = None):
        ...
        ax.plot(self.data['frame_number'], self.data['brightness'], 
                alpha=0.5, label='Raw')
        ...
        if window > 0:
            ...
            ax.plot(self.data['frame_number'], smoothed, 
    ...
    def plot_trajectory(self, output_file: Optional[str] = None):
        ...
        ax.plot(self.data['x'].iloc[0], self.data['y'].iloc[0], 
               'go', markersize=10, label='Start')
        ax.plot(self.data['x'].iloc[-1], self.data['y'].iloc[-1], 
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            if bleaching:
                bleach_df = pd.DataFrame([{
                    k: v for k, v in bleaching.items() 
                    if k != 'smoothed_brightness'
                }])
