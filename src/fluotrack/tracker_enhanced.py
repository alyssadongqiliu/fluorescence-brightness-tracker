...
class AdaptiveBackgroundModel:
    ...
    def update(self, frame: np.ndarray):
        ...
        if self.mean is None:
            ...
            ) * self.mean + self.learning_rate * frame

            # Running std
            diff = np.abs(frame - self.mean)
            self.std = (1 - self.learning_rate) * self.std + self.learning_rate * diff
...
class EnhancedBrightnessTracker:
    ...
    def __init__(
        ...
        logger.info(
            f"Enhanced tracker initialized: "
    ...
    def find_multiple_spots(
        self, frame: np.ndarray, num_spots: int = 5, min_distance: int = 10
    ) -> List[Dict[str, any]]:
        """
